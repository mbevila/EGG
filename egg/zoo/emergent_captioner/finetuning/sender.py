...
class BLIPSender(nn.Module):
    def __init__(
        ...
        model, vis_processors, _ = load_model_and_preprocess(name="blip_caption", model_type=blip_model)
    ...
    def forward(self, images: torch.Tensor, aux_input: Dict[Any, torch.Tensor] = None):
        ...
        with torch.no_grad():
            ...
            prompts = torch.tensor(self.prompt).unsqueeze(0).repeat(images.size(0), 1).to(self.device)
        ...
        log_probs = log_probs.gather(2, captions[:, self.prefix_len:].unsqueeze(2)).squeeze(-1)
